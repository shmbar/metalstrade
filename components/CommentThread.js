...
const AVATAR_COLORS = ['var(--brand)', 'var(--ok-text)', 'var(--warn-text)', 'var(--violet-text)', 'var(--bad-text)', 'var(--teal-text)'];
const colorFor = (key = '') => {
    let h = 0;
...
const CommentThread = ({ entityType, entityId, entityLabel }) => {
    ...
    return (
        <div className='flex flex-col p-3' style={{ minHeight: 240 }}>
                        ...
                        {comments.map((c) => {
                            ...
                            return (
                                <li key={c.id} className='flex items-start gap-2'>
                                    <span className='inline-flex items-center justify-center rounded-full text-[var(--on-brand)] flex-shrink-0 mt-0.5' style={{ width: 24, height: 24, fontSize: 'var(--fs-caption)', background: colorFor(c.authorName || c.authorUid) }}>
                                    ...
                                    <div className='min-w-0 flex-1'>
                                        ...
                                        <div className='rounded-lg px-2.5 py-1.5 mt-0.5 whitespace-pre-wrap break-words' style={{ fontSize: 'var(--fs-body)', color: 'var(--port-gore)', background: mine ? 'var(--bg-subtle)' : 'var(--bg-subtle)', border: '1px solid var(--bg-subtle)' }}>
                                            {c.text}
                                        </div>
                                    </div>
                                </li>
            ...
            {/* Composer */}
            <div className='flex items-end gap-2 mt-2 pt-2' style={{ borderTop: '1px solid var(--selago)' }}>
                <textarea
                    value={text}
                    ...
                    style={{ fontSize: 'var(--fs-input)', borderColor: 'var(--line)', background: 'var(--bg-subtle)', color: 'var(--port-gore)', fontFamily: 'inherit' }}
                />
                <button
                    onClick={send}
                    disabled={!text.trim() || sending}
                    className='flex items-center gap-1 px-3 py-2 rounded-2xl text-[var(--on-brand)] font-medium transition-all disabled:opacity-50'
