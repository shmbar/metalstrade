...
const Accounting = () => {
  ...
  useEffect(() => {
    ...
    const Load = async () => {
      ...
      for (let i = 0; i < dt.length; i++) {
        ...
        let item = {
          dateInv: l.final ? l.date : l.date.endDate,
  ...
  let propDefaults = Object.keys(settings).length === 0 ? [] : [
    ...
    { accessorKey: 'expType', header: getTtl('Expense Type', ln), bgt: 'bg-yellow-200', bgr: 'bg-yellow-50', cell: (props) => <p>{gQ(props.getValue(), 'Expenses', 'expType')}</p> },
