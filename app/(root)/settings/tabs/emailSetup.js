...
const EmailSetup = () => {
    ...
    return (
        <div className='p-3 space-y-3'>
            {/* Status card */}
            <div className='rounded-2xl p-4' style={{ border: '1px solid var(--line)', background: 'var(--bg-subtle)' }}>
                <div className='flex items-center justify-between mb-3'>
                    ...
                    <button
                        onClick={fetchStatus}
                        disabled={loading}
                        className='px-2.5 py-1 rounded-control border transition-colors hover:border-[var(--endeavour)] disabled:opacity-50'
                    ...
                    <>
                        {status.ready ? (
                            <div className='flex items-start gap-2 px-3 py-2.5 rounded-lg' style={{ background: 'var(--ok-bg)', border: '1px solid var(--ok-border)' }}>
                                ...
                                <div>
                                    <p className='font-semibold' style={{ fontSize: 'var(--fs-body)', color: 'var(--ok-text)' }}>
                                        Email sending is configured
                                    </p>
                                    <p style={{ fontSize: 'var(--fs-table)', color: 'var(--ok-text)', marginTop: '2px' }}>
                            ...
                            <div className='flex items-start gap-2 px-3 py-2.5 rounded-lg' style={{ background: 'var(--warn-bg)', border: '1px solid var(--warn-border)' }}>
                                <AlertTriangle className='w-4 h-4 flex-shrink-0 mt-0.5' style={{ color: 'var(--warn-text)' }} />
                                <div>
                                    <p className='font-semibold' style={{ fontSize: 'var(--fs-body)', color: 'var(--warn-text)' }}>
                                        Email sending is NOT configured
                                    </p>
                                    <p style={{ fontSize: 'var(--fs-table)', color: 'var(--warn-text)', marginTop: '2px' }}>
                                        Missing:{' '}
                ...
                <div className='flex items-center justify-between gap-3 flex-wrap'>
                    ...
                    <div className='flex items-center gap-1.5'>
                        <input
                            ...
                            className='w-16 text-center rounded-control border px-2 py-1 outline-none focus:border-[var(--endeavour)] focus:ring-2 focus:ring-[var(--endeavour)]/20'
                            style={{ fontSize: 'var(--fs-body)', borderColor: 'var(--line)', background: "var(--bg-card)", color: 'var(--port-gore)' }}
                        />
            ...
            {/* Setup steps */}
            <div className='rounded-2xl p-4' style={{ border: '1px solid var(--line)', background: "var(--bg-card)" }}>
