...
const InvoiceModal = () => {
	...
	const moveToContracts = async() => {
		let Con = await loadInvoice(uidCollection, 'contracts', valueInv.poSupplier)
		setIsOpen(false)
		
		setDateSelect({
			month: [valueInv.poSupplier.date.substring(6, 7).toString().padStart(2, '0')],
			year: valueInv.poSupplier.date.substring(0, 4)
		})
		...
		<div className="px-1">
			...
			<div className='grid grid-cols-3 gap-3 pt-2'>
				...
				<div className='col-span-12 md:col-span-1 border border-slate-300 p-2 rounded-lg'>
					...
					<div className={`flex gap-4 justify-between ${fnl ? 'py-0.5' : 'py-1.5'}`}>
						<p className={`flex items-center text-sm font-medium whitespace-nowrap ${fourthRule && 'text-slate-400'}`}>{thirdRule ? 'QTY Ingots' : getTtl('totalGross', ln)}:</p>
