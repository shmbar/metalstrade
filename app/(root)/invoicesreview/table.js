...
const Customtable = ({ data, datattl, propDefaults, SelectRow, lastAction, name, cb, valCur, filteredData,
	setFilteredData, setValCur, settings , excellReport}) => {
	...
	const showDetail = (obj, x, ttl) => {

		let showTotalCur = ttl.ttl
		...
		return x === 'client' ? obj.final ? obj.client.client : getD(tmp.arr, obj, x) :
			(x === 'date' && obj['date'].startDate !== null && !obj.final) ?
				...
				(x === 'date' && obj['date'].startDate === null && !obj.final) ?
					'' :
					(x === 'totalPrepayment1' || x === 'deviation'
						|| x === 'inDebt' || x === 'payments' || x === 'debtaftr' || x === 'debtBlnc'
						|| x === 'totalAmount') ? new Intl.NumberFormat('en-US', {
							style: 'currency',
							currency: showTotalCur ? getD(settings.Currency.Currency, valCur, 'cur') :
								obj.final ? obj.cur.cur : getD(settings.Currency.Currency, obj, 'cur'),
		...
		const newObj = data.map((obj) => {
			...
			for (let key of Object.keys(obj)) {
				if (tmpArr.includes(key) || key === 'id') {
					tmp[key] = showDetail(obj, key, { ttl: false })
				}
			}
			return tmp;
		});
