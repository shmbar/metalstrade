...
const Customtable = ({ data, propDefaults, SelectRow, lastAction, name, cb,cb1, filteredData,
	setFilteredData,  settings, type, excellReport }) => {
	...
	const showDetail = (obj, x, ttl) => {
		...
		return (x === 'unitPrc' || x === 'total') ? new Intl.NumberFormat('en-US', {
			style: 'currency',
			currency: getD(settings.Currency.Currency, obj, 'cur'),
			minimumFractionDigits: 2
		}).format(obj[x]) :
			(x === 'supplier' || x==='stock') ? getD(tmp.arr, obj, x) :
				obj[x]
	}
		...
		<div className="flex flex-col relative border rounded-lg ">
			<div  >
				...
				<Header cols={cols} data={filteredData} name={name}
					...
					setCols={setCols} cb={cb} cb1={cb1} type={type} excellReport={excellReport}/>
