...
const Shipments = () => {
    ...
    let propDefaults = Object.keys(settings).length === 0 ? [] : [
        ...
        {
            accessorKey: 'supInvoices', header: getTtl('Supplier inv', ln), cell: (props) => { const arr = Array.isArray(props.getValue()) ? props.getValue() : [props.getValue()]; return <div>{arr.map((item, i) => { const s = String(item ?? ''); const display = s.length > 22 ? s.slice(0, 22) + '…' : s; return <div key={i} title={s.length > 22 ? s : undefined} className={i < arr.length - 1 ? 'border-b border-[var(--rock-blue)] py-0.5' : 'py-0.5'}>{display}</div>; })}</div>; },
                ...
                <div>
                    ...
                    <div className="border border-[#b8ddf8] rounded-xl p-4 mt-8 shadow-lg bg-white relative">
                        ...
                        <div className='flex items-center justify-between flex-wrap'>
                            <h1 className="text-[var(--chathams-blue)] font-poppins responsiveTextTitle font-medium border-l-4 border-[var(--chathams-blue)] pl-2">{getTtl('Invoices Statement', ln)}</h1>
