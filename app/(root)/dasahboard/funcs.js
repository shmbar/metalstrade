...
const setPieArrs = (arr) => {
    ...
    arrTmp = Object.entries(arrTmp);
    arrTmp.sort((a, b) => b[1] - a[1]);
...
export const setMonthsInvoices = (data, settings) => {
    ...
    data.forEach(obj => {
        ...
        let month = !obj.final ? dateFormat(obj.date.startDate, 'm') * 1 : dateFormat(obj.date, 'm') * 1
...
export const calContracts = (data, settings) => {
    ...
    data.map((x, i) => {
        ...
        accumulatedPmnt[dateFormat(x.date.startDate, 'm') * 1] += tmp;
        //top 5 suppliers
        ...
        x.expenses.forEach(obj => {
            if (obj) {
                ...
                if (obj && !isNaN(parseFloat(obj.amount))) {
                    accumulatedExp[dateFormat(x.date.startDate, 'm') * 1] += parseFloat(obj.amount * 1 * mltTmp);
                }
            };
        });
    })
