...
interface StreamArgs {
  ...
  /** the rest of that final event: a ranking to draw as bars, and follow-up questions */
  onStructure?: (structure: AssistantStructure) => void;
  signal?: AbortSignal;
}

/** A ranking under an answer ("which client owes the most?") — app/api/assistant route. */
...
export interface AssistantStructure {
  ...
  /** at most three questions another tool can actually answer */
  followUps?: string[];
}
...
export async function streamAssistant({ messages, currentData, currentPage, dateRange, onText, onSources, onStructure, signal }: StreamArgs): Promise<void> {
  ...
  if (!reader) {
    ...
    parseSse(text, onText, onSources, onStructure);
    return;
  }
  ...
  while (true) {
    ...
    buffer = lines.pop() || '';
    for (const line of lines) handleLine(line, onText, onSources, onStructure);
  }
  if (buffer.trim()) handleLine(buffer.trim(), onText, onSources, onStructure);
}
...
function handleLine(
  ...
  onStructure?: (s: AssistantStructure) => void
) {
  if (!line.startsWith('data: ')) return;
  const payload = line.slice(6).trim();
  if (payload === '[DONE]') return;
  try {
    const { text, error, sources, ranking, followUps } = JSON.parse(payload);
    if (error) throw new Error(error);
    if (text) onText(text);
    // The final event also carries a ranking and follow-up questions (web 8d5a9cf6).
    // Mobile parsed only the citations, so "who owes the most?" came back as prose
    // with no bars and no next question to tap.
    const hasRanking = !!ranking && Array.isArray(ranking.rows) && ranking.rows.length > 1;
    const hasFollowUps = Array.isArray(followUps) && followUps.length > 0;
    if (hasRanking || hasFollowUps) {
      onStructure?.({ ranking: hasRanking ? ranking : null, followUps: hasFollowUps ? followUps : [] });
    }
...
function parseSse(
  ...
  body.split('\n\n').forEach((line) => handleLine(line.trim(), onText, onSources, onStructure));
}
