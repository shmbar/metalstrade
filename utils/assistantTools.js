// The assistant's prompt, tool schemas and tool implementations.
//
// These live outside app/api/assistant/route.js because an App Router route file
// may only export the route fields Next recognises (GET/POST/dynamic/…) — adding
// an `export function executeTool` there compiles in dev and then fails
// `npm run build` with "not a valid Route export field". Keeping the logic in a
// plain module means the route stays a thin transport layer and the tools can be
// unit-tested directly (see __tests__/assistantTools.test.js).
export const SYSTEM_PROMPT = `You are an intelligent assistant for IMS (Inventory Management System) - a metals trading and inventory management platform.
